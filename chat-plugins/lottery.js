	...
	lottery: function (target, room, user) {
		...
		case 'create':
			...
			if (parts[2] === 'pmall') {
				...
				BH.pmall('/html ' + loto_notification, '~Spark Lottery');
		...
		case 'setlimit':
			if (!this.can('hotpatch')) return false;
			if (!BH.lottery.gameActive) return this.errorReply("The game of lottery is not currently running.");
			if (BH.lottery.players.length >= 1) return this.errorReply("You cannot change the limit because someone(s) have already bought a lottery ticket.");
