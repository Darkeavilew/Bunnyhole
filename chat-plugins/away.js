...
exports.commands = {
	away: function (target, room, user) {
		if (!user.isAway && user.name.length > 19) return this.sendReply('Your user name is too long to update your status.');
		...
		if (user.can('lock', null, room)) this.add('|raw|-- ' + BH.nameColor(user.name).bold() + ' is now ' + target.toLowerCase() + '.');
	...
	back: function (target, room, user) {
		...
		user.updateIdentity();
		user.isAway = false;
		if (user.can('lock', null, room)) this.add('|raw|-- ' + BH.nameColor(newName).bold() + '  is no longer ' + status.toLowerCase() + '.');
	},
