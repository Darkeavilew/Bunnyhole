...
exports.commands = {
	...
	'!tell': true,
	 tell: function (target, room, user, connection, cmd) {
		...
		let msg = '<small>[' + {timestamp} + ']</small> ' + userSymbol + '<strong class="username"><span style = "color:' + BH.Color(user.userid) + '">' + user.name + ':</span></strong> ' + Chat.escapeHTML(target);
		if (tell) {
		...
		if (this.message.startsWith(`/tell`)) {
			user.send('|pm| ' + this.targetUsername + '|' + this.user.getIdentity() + '|/raw ' + '<small>[' + {timestamp} + ']</small>' + userSymbol + '<strong class="username"><span style = "color:' + BH.Color(user.userid) + '">' + user.name + ':</span></strong> ' + Chat.escapeHTML(target));
