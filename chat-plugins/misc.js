	...
	'!tell': true,
	 tell: function (target, room, user, connection, cmd) {
		...
		if (this.message.startsWith(`/tell`)) {
			user.send('|pm| ' + this.targetUsername + '|' + this.user.getIdentity() + '|/raw ' + '<small>[' + ${().format("HH:mm:ss")} + ']</small>' + userSymbol + '<strong class="username"><span style = "color:' + BH.Color(user.userid) + '">' + user.name + ':</span></strong> ' + Chat.escapeHTML(target));
