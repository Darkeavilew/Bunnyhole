...
class Wheel {
	...
	endDisplay(winner, winnings) {
		...
		BH.nameColor(winner, true) + ' has won ' + winnings + '</div>';
	}
}
		...
		join: function (target, room, user) {
			...
			Economy.readMoney(user.userid, money => {
				...
				Economy.writeMoney(user.userid, (room.wheel.wheel[room.wheel.wheel.length - 1] * room.wheel.wheelMulti) * -1, function () {
					room.add('|raw|<b><font color="orange">' + BH.nameColor(user, true) + ' has spun the wheel.</font></b>');
			...
			delete room.wheel;
			room.add('|raw|<font color="orange">The WOM was ended by <b>' + BH.nameColor(user, true) + '</b>.</font>'); //html needed
		},
